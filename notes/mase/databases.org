...
** Procedure
   ...
   #+end_src
* Dynamic SQL
  - It works like prepared statements in Java
    #+begin_src sql
      delimiter //
...
** Incorrect data type management
   ...
   #+end_src
** Blind SQL injection
   - Relies on server responses to deduce the inner workings. Usually uses timings or boolean response.
   - One can start with simple queries "Does the first character of the admin’s password hash start with A". Then based on the timing we may know if it is a yes or a no. And so on so forth.
...
**** Dirty Reads
     - Accessing data that hasn't been committed yet. It's even worse when the first transaction rolls back, so the other transaction read something that shouldn't even exist.
**** Lost Updates
...
**** Phantom Reads
     - A transaction re-runs a query and gets different results that operate on multiple rows, another committed transaction has inserted additional rows or deleted some row that satisfy the search criteria. So now when the first transaction runs the query again it gets a different result.
*** Solution
